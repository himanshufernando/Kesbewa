...
import tkhub.project.kesbewa.databinding.ListviewCartitemsBinding
...
class CartItemAdapter : ListAdapter<CartItem, RecyclerView.ViewHolder>(CartItemDiffCallback()) {
    ...
    interface ClickListener {
        fun onClick(cartitemsellect: CartItem, aView: View)
    }
    class CartItemViewHolder(private val binding: ListviewCartitemsBinding ,var mClickListener: ClickListener ) :
        RecyclerView.ViewHolder(binding.root) {
        init {
            binding.setClickListener { binding.cartitem?.let { selectedProduct -> mClickListener.onClick(selectedProduct,it) } }
        }
